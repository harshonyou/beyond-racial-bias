...
#         return final_color, shaded_lambertian, shaded_specular
...
class LambertianShader(nn.Module):
    ...
    def forward(
        ...
        light_directions = light_directions.to(self.device)
        light_colors = light_colors.to(self.device)
...
class BlinnPhongShader(nn.Module):
    ...
    def forward(
        ...
        light_directions = light_directions.to(self.device)
        light_colors = light_colors.to(self.device)
        specular = specular.to(self.device)
        shininess = shininess.to(self.device)
        view_directions = view_directions.to(self.device)
