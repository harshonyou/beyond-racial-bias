...
from demos.targetted import PhotometricFitting
...
def process_image(item_path, save_folder, device_name):
    """Processes a given image and performs photometric fitting."""
    # Prepare filenames for output
    save_name = os.path.split(item_path)[-1]
    ...
    for i in range(0, 3):
        image_path = f'{item_path}/{i}.png'
        img = cv2.imread(image_path)
        imgs.append(img)

    # Run the fitting process
    fitting.run(imgs, seg_net, face_detect, face_landmark, cfg.rect_thresh, save_name, save_folder, video_flag=False)
...
def log_ita_to_csv(csv_path, image_name, idx, ita_gt, ita_gen, ita_error, skin_type, predicted_skin_type):
    """Logs the ITA values, error, and skin type for an image to a CSV file."""
    fieldnames = ['image_name', 'idx', 'ita_gt', 'ita_gen', 'ita_error', 'skin_type', 'predicted_skin_type']
...
if __name__ == '__main__':
    ...
    for item in os.listdir(benchmark_root):
        ...
        if check_if_image_processed(csv_path, item):
            print(f"Skipping already processed image: {item}")
            continue

        print(f"Processing {item}")

        log_file_path = os.path.join(output_path, f"{item}.log")
        ...
        with open(log_file_path, 'w') as log_file:
            ...
            with redirect_stdout(log_file):
                ...
                process_image(item_path, output_path, device_name)

            sys.stdout = original_stdout  # Restore stdout to original

            print(f"Successfully processed {item}, log saved to {log_file_path}")


        for idx in range(0, 3):
            generated_image_path = os.path.join(output_path, f"{item}_{idx}.png")
            ground_truth_image_path = os.path.join(item_path.replace("crops", "crop-albedos"), f"{idx}.png")
            ...
            print(f"ITA Error for {item}_{idx}: {mean_ita_difference}")

            log_ita_to_csv(csv_path, item, idx, ita_gt, ita_predicted, mean_ita_difference, skin_type, predicted_skin_type)

            print(f"Processed face {idx + 1}/{3}: {item}_{idx}")

        # print(f"Processed image {processed_images + 1}/{batch_size}: {item}")
