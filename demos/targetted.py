...
class PhotometricFitting(object):
    ...
    def optimize(self, images, landmarks, image_masks, video_writer):
        ...
        for k in range(50):
            ...
            if not skip:
                ...
                if non_rigid_mode:
                    grids['albedos'] = torch.clamp(F.interpolate(
                    ...
                    ), 0, 1).squeeze().detach().cpu()
                    grids['albedo_renders'] = torch.clamp(F.interpolate(
                        torchvision.utils.make_grid(torch.cat(all_albedo_renders, dim=0), nrow=num_faces, padding=0).unsqueeze(0),
                        size=(grids['images'].shape[1], grids['images'].shape[2]), mode='bilinear', align_corners=False
                    ), 0, 1).squeeze().detach().cpu()
                    grids['renders'] = torch.clamp(F.interpolate(
    ...
    def run(self, imgs, net, rect_detect, landmark_detect, rect_thresh, save_name, savefolder, video_flag=True):
        all_images = []
        all_landmarks = []
        all_image_masks = []
        ...
        for img in imgs:
            ...
            if images:
                ...
                # Append results for this image to the overall list
                all_images.append(images)
                all_landmarks.append(landmarks)
                all_image_masks.append(image_masks)
        ...
        all_image_masks = torch.cat(all_image_masks, dim=0)
        ...
        if video_flag:
            ...
            frame_size = (cfg.image_size * len(all_images), cfg.image_size * 9)
        ...
        params = self.optimize(all_images, all_landmarks, all_image_masks, video_writer)
        ...
        for idx in range(len(all_images)):
            ...
            img = all_images[idx].detach().cpu().numpy().transpose(1, 2, 0)
...
if __name__ == '__main__':
    ...
    path = 'benchmarks/FAIR_benchmark/validation_set/crops/ag_face_triplegangers_3_300_000005'

    device_name = 'cuda'

    save_name = os.path.split(path)[-1]
    ...
    fitting.run(imgs, seg_net, face_detect, face_landmark, cfg.rect_thresh, save_name,
                cfg.save_folder)
