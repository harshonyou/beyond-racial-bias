...
def draw_train_process(title, iters, loss, label_loss):
    ...
    # Convert each tensor in the loss list to a numpy array after moving it to CPU
    loss_np = [l.cpu().detach().numpy() for l in loss]
    plt.plot(iters, loss_np, color='red', label=label_loss)
    plt.legend()
    plt.grid()
    plt.show()